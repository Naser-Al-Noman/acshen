...
const CURRENT_ROLE_CHUNK_ID = 'experience-nexbild-technologies';

function isEmploymentQuery(message) {
  return /\b(work|works|working|job|jobs|employ|employer|employed|company|companies|role|roles|career|where.*(he|she|noman|naser)|current(ly)?|full[- ]?time|office|hired)\b/i.test(
    String(message || '')
  );
}

function ensureCurrentRoleChunk(chunks, limit) {
  const list = Array.isArray(chunks) ? [...chunks] : [];
  if (list.some((c) => c && c.id === CURRENT_ROLE_CHUNK_ID)) {
    return list.slice(0, limit);
  }

  const all = loadLocalChunks();
  const current = all.find((c) => c.id === CURRENT_ROLE_CHUNK_ID);
  if (!current) return list.slice(0, limit);

  // Keep current role first so the model treats it as present employment.
  return [current, ...list.filter((c) => c.id !== CURRENT_ROLE_CHUNK_ID)].slice(0, limit);
}
...
function selectLocalChunks(message, limit) {
  ...
  const scored = chunks.map((chunk) => {
    ...
    // Prefer current role for employment questions.
    if (chunk.id === CURRENT_ROLE_CHUNK_ID && isEmploymentQuery(message)) score += 2;
    return { chunk, score };
  });
  ...
  const selected = top.length
    ? top.map((s) => s.chunk)
    : chunks.filter((c) => c.id === 'profile').slice(0, 1);

  return isEmploymentQuery(message) ? ensureCurrentRoleChunk(selected, limit) : selected;
}
...
async function getPortfolioChunks(message) {
  ...
  if (!USE_VECTOR_SEARCH) {
    chunks = selectLocalChunks(message, MAX_LOCAL_CHUNKS);
  } else {
    ...
    if (VECTOR_BACKEND === 'local') {
      ...
      chunks = await retrieveChunks(queryEmbedding, MAX_LOCAL_CHUNKS, VECTOR_THRESHOLD);
    }
  }

  if (isEmploymentQuery(message)) {
    return ensureCurrentRoleChunk(chunks, MAX_LOCAL_CHUNKS);
  }
  return chunks;
}
...
function buildSystemInstruction(chunks) {
  ...
  return (
    ...
    'Employment rule: his CURRENT job is Software Quality Assurance Engineer (Full-Time) at ' +
    'Nexbild Technologies Bangladesh Ltd. (since September 2026). ' +
    'SJ Innovation LLC, Render Lab, and earlier roles are PAST experience only — never call them current.\n\n' +
    'PORTFOLIO CONTEXT:\n' +
    context
  );
}
